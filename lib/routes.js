	...
	(async () => {
		...
		try {
			...
			app.post('/amazonCustomerSync', salesLogin, async function (req, res, next) {
				...
				if (!Aqxolt_Order_Profile && !Aqxolt_Customer && !RefreshToken || !ClientId || !ClientSecret || !AWSAccessKey || !AWSSecretAccessKey || !AWSSellingPartnerRole) {
					...
					console.log('Response BuyerInfo ->', JSON.stringify(BuyerDetails));
					 ...
					 if (noBuyerEmail != []) {
						 for (let i in noBuyerEmail) {
							 var email = noBuyerEmail[i].BuyerEmail;
							 var byEmail = email.split('@', 1);
					...
					for (let i in allBuyerDetails) {
						email.push(allBuyerDetails[i].ERP7__Email__c)
					}
					...
					function conInsertion() {
						conn.query("SELECT Id, Name, ERP7__Email__c, ERP7__Order_Profile__c, ERP7__Account_Profile__c, ERP7__Account_Type__c FROM Account WHERE ERP7__Email__c IN ('" + email.join("','") + "')", function (err, result) {
							if (err) { return console.error(err); }
			...
			app.post('/amazonOrderSync', salesLogin, async function (req, res, next) {
				...
				conn.query("SELECT Id, AccountId FROM Order WHERE ERP7__E_Order_Id__c IN ('" + AmazonOrderIdList.join("','") + "')", function (err, result) {
					if (err) { return console.error(err); }
