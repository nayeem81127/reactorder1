			...
			app.post('/amazonProductSync', salesLogin, async function (req, res, next) {
				...
				if (OrderItemsList != []) {
					...
					conn.query("SELECT Id, ERP7__Submitted_to_Amazon__c, amazon_ext_id__c, ERP7__Amazon__c, Name, ERP7__SKU__c, ERP7__ASIN_Code__c, ERP7__Price_Entry_Amount__c,IsActive, ERP7__Track_Inventory__c FROM product2", function (err, result) {
						...
						conn.query("SELECT Id, product2Id FROM pricebookentry WHERE isactive = true ORDER BY lastmodifieddate", function (err, result) {
							if (err) { return console.error(err); }
							console.log('id include'+result.records[i].product2Id)
							for (let i in result.records) {
							...
							for (let i in mainProductList) {
								if (priceBookMap.has(mainProductList[i].Id)) {
									...
									// conn.sobject("pricebookentry").update({ 
									// 	UnitPrice: `'${priceBookEntriesObjectList[i].unitprice}'`,
									// 	Pricebook2Id: `'${PriceBookId}'`
									//   }, function(err, ret) {
				...
				/*	pool.connect();
					pool.query('SELECT * FROM salesforce.product2', (err, ress) => {
						...
						//console.log('OrderItemsList75->',JSON.stringify(ress.rows));
						if (productList.length > 0 && OrderItemsList.length > 0) {
							for (let i in productList) {
								if (productList[i].erp7__orderitemid__c != null && productList[i].erp7__orderitemid__c != "") {
									for (let j in OrderItemsList) {
										if (productList[i].erp7__orderitemid__c == OrderItemsList[j].ASIN) {
											if (!mainProductIdList.includes(productList[i].sfid)) mainProductList.push(productList[i]);
						...
						pool.connect();
						pool.query('SELECT * FROM salesforce.pricebookentry WHERE isactive = true ORDER BY lastmodifieddate', (err, resp) => {
							if (err) throw err;
							...
							for (let i in mainProductList) {
								if (priceBookMap.has(mainProductList[i].sfid)) {
									...
									console.log('randomId->', randomId);
									for (let i in priceBookEntriesObjectList) {
										//INSERT INTO salesforce.product2(ERP7__OrderItemId__c, ERP7__Amazon__c, Name, ERP7__SKU__c, ERP7__ASIN_Code__c, ERP7__Price_Entry_Amount__c,IsActive,ERP7__Track_Inventory__c)VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (ERP7__OrderItemId__c) DO UPDATE SET ERP7__Amazon__c=$2
										pool.query('UPDATE salesforce.pricebookentry SET UnitPrice=$1 WHERE Pricebook2Id=$2', [`${priceBookEntriesObjectList[i].unitprice}`, `${PriceBookId}`], (err, res1) => {
											if (err) {
												console.log("Error-> Failed to insert data into pricebookentry");
												console.log(err);
											}
											else {
												console.log('DB upsert data into pricebookentry->', res1);
											}
									...
									console.log('randomId->', randomId);
									for (let i in priceBookEntriesObjectList) {
										pool.query(`INSERT INTO salesforce.pricebookentry(IsActive, UnitPrice, Pricebook2Id, Product2Id, ERP7__ExternalId__c)VALUES($1,$2,$3,$4,$5)`, [`${isActive}`, `${priceBookEntriesObjectList[i].UnitPrice}`, `${PriceBookId}`, `${priceBookEntriesObjectList[i].Product2Id}`, `${randomId}`], (err, res2) => {
											if (err) {
												console.log("Error-> Failed to insert data into pricebookentry");
												console.log(err);
											}
											else {
												console.log('DB upsert data into pricebookentry->', res2);
											}
