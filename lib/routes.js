			...
			app.get('/shopify', checkNotAuthenticated, isShopify, isAccess, async function (req, res, next) {
				...
				await JSON.stringify(client.query('SELECT * FROM shops WHERE email=$1', [email], function (err, result) {
					...
					if(result.rows.length == 0){
						...
						client.query('SELECT * FROM "jobs_log" WHERE "email"=$1 AND category=$2 ORDER BY id DESC LIMIT 25', [email, 'shopify'], function (err, result) {
							if (err) {
								console.log('error', err);
								throw err;
							}
							else {
								return res.render('shopify', { generateLog: result, shopDetails: shopDetails, error_msg, success_msg, username, manage, amaz, etsy, shopify, majento, wooCommerce, bigCommerce, quickBooks, ebay });
							}
						})
					}
				}));
