	...
	(async () => {
		...
		try {
			...
			app.post('/amazonOrderSync', salesLogin, async function (req, res, next) {
				...
				var AllPbeList = [];
				pool.connect();
				pool.query('SELECT * FROM salesforce.pricebookentry WHERE isactive = true ORDER BY lastmodifieddate', (err, resp12) => {
					if (err) throw err;
					for (let i in resp12.rows) {
						if (mainProductIdList.includes(resp12.rows[i].product2id)) {
							AllPbeList.push(resp12.rows[i]);
						}
					}
				})
