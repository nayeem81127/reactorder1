...
module.exports = function (app) {
	...
	(async () => {
		...
		try {
			...
			app.get('/shopify', checkNotAuthenticated, isShopify, isAccess, MySession, function (req, res, next) {
				res.render('shopify', { username, manage, amaz, etsy, shopify, majento, wooCommerce, bigCommerce, quickBooks, ebay });
			});

			app.get('/installExtension', checkNotAuthenticated, isAccess, MySession, async function (req, res, next) {				
			...
			// app.post('webhooks/customer/data_request', (req, res, next) => {
			// 	res.sendStatus(200)
			// })

			// app.post('webhooks/customer/redact', (req, res, next) => {
			// 	res.sendStatus(200)
			// })

			// app.post('webhooks/shop/redact', (req, res, next) => {
			// 	res.sendStatus(200)
			// })

			app.post("/webhooks/customer/data_request", (req, res) => {
				console.log(req.body) 
				res.status(200).end() 
			  })

			  app.post("/webhooks/customer/redact", (req, res) => {
				console.log(req.body) 
				res.status(200).end() 
			  })
