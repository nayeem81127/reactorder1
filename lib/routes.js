...
module.exports = function (app) {
	...
	app.post('/updateAqxolt', async function (req, res, next) {
		...
		await JSON.stringify(client.query('SELECT id FROM "users" WHERE "email"=$1', [email], function (err, result) {
			if (result.rows.length > 0) {
				client.query(`UPDATE "users" set aqxolt_client=$1, client_secret=$2, aqxolt_channel=$3, aqxolt_order_profile=$4, aqxolt_customer=$5 WHERE email=$6`, [aqxolt_client, client_secret, aqxolt_channel, aqxolt_order_profile, aqxolt_customer, email]),
					client.query('COMMIT');
	...
	app.post('/edit', async function (req, res, next) {
		...
		console.log( aqxoltClient, clientSecret, aqxoltChannel,'aqxoltOrderProfile' +aqxoltOrderProfile, aqxoltCustomer, 'email'+ emailId );
		const client = await pool.connect();
		await client.query('BEGIN');
		await JSON.stringify(client.query(`UPDATE "users" set aqxolt_client=$1, client_secret=$2, aqxolt_channel=$3, aqxolt_order_profile=$4, aqxolt_customer=$5 WHERE email=$6`, [aqxoltClient, clientSecret, aqxoltChannel, aqxoltOrderProfile, aqxoltCustomer, emailId]), 			
				client.query('COMMIT'));
				client.release();
				req.flash('success_msg', '• Record is Updated');
				res.redirect('/manage')
	...
	app.get('/manage-edit', checkNotAuthenticated, isAccess, function (req, res, next) {
		res.render('manage-password', { username, manage, amaz, etsy, shopify, majento, wooCommerce, bigCommerce, quickBooks, ebay });
	});
