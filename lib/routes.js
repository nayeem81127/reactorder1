			...
			app.get('/amazon/:id', checkNotAuthenticated, isAmazon, isAccess, async function (req, res) {
				...
				if (idValue == 'All') {
					...
					await JSON.stringify(client.query('SELECT * FROM amazon_credentials WHERE "email"=$1', [email], function (err, result) {
						...
						if (result.rows.length > 0) {
							var amazDetails = result;
							client.query('SELECT * FROM "jobs_log" WHERE "email"=$1 AND amazon_app_client_id=$2 AND category=$3 ORDER BY updated_at DESC', [email, idValue, 'amazon'], function (err, result) {
								if (err) {
									console.log('error', err);
									throw err;
								}
