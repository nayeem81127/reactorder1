	...
	(async () => {
		...
		try {
			...
			app.get('/amazon', checkNotAuthenticated, isAmazon, isAccess, async function (req, res, next) {
				...
				await JSON.stringify(client.query('SELECT * FROM amazon_credentials WHERE "email"=$1', [email], function (err, result) {
					...
					if (result.rows.length == 0) {
						...
						client.query('SELECT * FROM "jobs_log" WHERE "email"=$1 AND category=$2 ORDER BY updated_at DESC LIMIT 25', [email, 'amazon'], function (err, result) {
							if (err) {
								console.log('error', err);
								throw err;
							}
			...
			app.get('/amazon/:id', checkNotAuthenticated, isAmazon, isAccess, async function (req, res) {
				...
				if (idValue == 'All') {
					...
					await JSON.stringify(client.query('SELECT * FROM amazon_credentials WHERE "email"=$1', [email], function (err, result) {
						...
						if (result.rows.length > 0) {
							var amazDetails = result;
							client.query('SELECT * FROM "jobs_log" WHERE "email"=$1 AND amazon_app_client_id=$2 AND category=$3 ORDER BY updated_at DESC LIMIT 25', [email, idValue, 'amazon'], function (err, result) {
								if (err) {
									console.log('error', err);
									throw err;
								}
			...
			app.delete('/deleteSeller/:id', async function (req, res) {
				...
				const client = await pool.connect();
				await client.query('BEGIN');
				await JSON.stringify(client.query(`Delete FROM amazon_credentials WHERE "email"=$1 And amazon_app_client_id=$2`, [email, req.params.id]),
					client.query('COMMIT'));
