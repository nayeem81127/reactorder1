                  ...
                  if (!Aqxolt_Order_Profile && !accessToken || !shopName) {
                    ...
                    for (let i in ProductArray) {
                      if (ProductArray[i].id != '' && ProductArray[i].id != undefined && ProductArray[i].variants[0].sku != '' && ProductArray[i].variants[0].sku != undefined) {
                        SkuId.push(ProductArray[i].variants[0].sku)
                        var img;
                      ...
                      setTimeout(async function () {
                        conn.query(`SELECT Id, ERP7__Price_Book__c FROM ERP7__Profiling__c where Id='${Aqxolt_Order_Profile}'`, function (err, result) {
                          ...
                          if (result.records.length == 0) {
                            ...
                            if (SkuId.length > 0) {
                              setTimeout(async function () {
                                conn.query("SELECT Id, Name, ERP7__SKU__c, ERP7__Manufacturer__c, Description, ProductCode, ERP7__Picture__c, StockKeepingUnit, ERP7__Price_Entry_Amount__c FROM Product2 WHERE StockKeepingUnit IN ('" + SkuId.join("','") + "')", function (err, result) {
                                  if (err) {
                                    var error = JSON.stringify(err);
                                    var obj = JSON.parse(error);
                                    if (obj.name == 'INVALID_SESSION_ID') {
                                  ...
                                  if (result.records.length == 0) {
                                    ...
                                    for (let i in result.records) {
                                      ...
                                      for (let j in ProductDetails) {
                                        if (result.records[i].StockKeepingUnit == ProductDetails[j].StockKeepingUnit) {
                                          var list = {
                                    ...
                                    // console.log(idval)
                                    // conn.sobject("Product2").del(idval
                                    //   ,
                                    //   function (err, rets) {
