                            ...
                            setTimeout(async function () {
                                if (Email === result.rows[z].email) {
                                    ...
                                    if (!Aqxolt_Order_Profile && !Aqxolt_Customer && !accessToken || !shopName) {
                                        ...
                                        console.log(CustomerDetails.length, buyerEmailInfo.length)
                                            ...
                                            setTimeout(async function () {
                                                conn.query("SELECT Id, Name, Email, AccountId, ERP7__Contact_External_Id__c FROM Contact WHERE AccountId IN ('" + accIdExist.join("','") + "')", function (err, result) {
                                                    ...
                                                    if (result.records.length > 0) {
                                                            ...
                                                            for (let j in address) {
                                                                if (result.records[i].ERP7__Contact_External_Id__c == address[j].customer_id) {
                                                                    if (address[j].zip != null && address[j].zip != '' || address[j].address1 != null && address[j].address1 != '') {
                                                                        ...
                                                                        AddressName.push(address[j].zip + ' ' + address[j].address1)
                                                                    }
                                                                }
                                                            }
                                                        }
                                                        ...
                                                        var addNotExist = [];
                                                        setTimeout(async function () {
                                                            conn.query("SELECT Id, Name, ERP7__Customer__c, ERP7__Contact__c, ERP7__City__c, ERP7__Country__c, ERP7__Postal_Code__c, ERP7__State__c FROM ERP7__Address__c WHERE ERP7__Customer__c IN ('" + accIdExist.join("','") + "') AND ERP7__Address_Line1__c IN ('" + AddressName.join("','") + "')", function (err, result) {
                                                                if (err) {
                                                                    var error = JSON.stringify(err);
                                                                    var obj = JSON.parse(error);
                                                                    if (obj.name == 'INVALID_SESSION_ID') {
