                                    ...
                                    if (!orderProfile && !RefreshToken || !ClientId || !ClientSecret || !AWSAccessKey || !AWSSecretAccessKey || !AWSSellingPartnerRole) {
                                        ...
                                        if (resS) {
                                            ...
                                            if (orderProfile != null) {
                                                setTimeout(async function () {
                                                    conn.query(`SELECT Id FROM ERP7__Profiling__c where Id='${orderProfile}'`, function (err, result) {
                                                        if (err) {
                                                            var error = JSON.stringify(err);
                                                            var obj = JSON.parse(error);
                                                                        ...
                                                                        if (priceBookEntryAvail != []) {
                                                                            conn.sobject("pricebookentry").create(priceBookEntryAvail,
                                                                                async function (err, rets) {
                                                                                    ...
                                                                                    pool.query('INSERT INTO jobs_log (email, updated_at, category, message, amazon_app_client_id) VALUES ($1, $2, $3, $4, $5, $6)', [Email, updatedDate, 'amazon', 'Product Sync', ClientId]);
